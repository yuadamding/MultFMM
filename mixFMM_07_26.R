mixfmm_07_26 = function(X = NULL ,
                        ...
                        maxstep = 100) {
  {
    ...
    pp = gauher(8)
    maxstep = 100
    if (is.null(lambda_gamma) | is.null(lambda_cnlm)) {
      temp = ifelse(is.null(lambda_gamma), lambda_cnlm, lambda_gamma)
      lambda_gamma = temp
      lambda_cnlm = temp
    }
    ...
    # j_list_unit = c(
    #   c(rep(4, 10), rep(5, 5), rep(5, 10), rep(4, 10), rep(4, 5)),
    #   c(rep(4, 10), rep(5, 5), rep(5, 10), rep(4, 10), rep(4, 5)),
    #   c(rep(4, 10), rep(5, 5), rep(5, 10), rep(4, 10), rep(4, 5)),
    #   c(rep(4, 10), rep(5, 5), rep(5, 10), rep(4, 10), rep(4, 5)),
    #   c(rep(4, 10), rep(5, 5), rep(5, 10), rep(4, 10), rep(4, 5))
    # )
    #j_list = list(j_list_unit, j_list_unit, j_list_unit, j_list_unit, j_list_unit, j_list_unit, j_list_unit, j_list_unit)
    rc_list = rep(2, 10)
    k = 3
    eps_nlm = 1e-3
  ...
  while ((hh < it) & (ratio > eps)) {
    ...
    if (init == T) {
      ...
      if (mc != 0) {
        ...
        for (i in 1:mc) {
          init_ordinal = list.append(init_ordinal,
                                     ordinal_init(x = Xc[[i]], rc_list, j_list[i][[1]], k))
        }
      }
      init = F
    }
    ...
    if (!is.null(X)) {
      ...
      for (num_m in 1:m) {
        if ((k == 1) & (r_list[num_m] == 1)) {
          ...
          if (sum(diag(sigma.tot[1, 1:p[num_m], 1:p[num_m], num_m])) < 0) {
            change = TRUE
          }#####
          if (det(sigma.tot[1, 1:p[num_m], 1:p[num_m], num_m]) < 0) {
            change = TRUE
          }########
        } else{
          for (i in 1:k) {
            ...
            if (sum(diag(sigma.tot[i, 1:p[num_m], 1:p[num_m], num_m])) < 0) {
              change[i] = TRUE
            }########
            if (det(sigma.tot[i, 1:p[num_m], 1:p[num_m], num_m]) < 0) {
              change[i] = TRUE
            }#############
          }
        }
        ...
        if ((k == 1) & (r_list[num_m] == 1)) {
          ...
          if (!is.null(Z)) {
            roy[, 1:r_list[num_m], , num_m] = chsi[, 1:r_list[num_m], 1:r_list[num_m], num_m] %*% (
              t(as.matrix(H[1:p[num_m], 1:r_list[num_m], num_m])) %*% ginv(psi[1:p[num_m], 1:p[num_m], num_m]) %*% t(X[, 1:p[num_m], num_m] - Z %*% t(B[1:p[num_m], , num_m])) + solve(sigma[, 1:r_list[num_m], 1, num_m]) %*% beta[1, 1:r_list[num_m], num_m] %*% t(x.z)
            )
          } else{
          ...
          Ezz.hy[1, 1:r_list[num_m], 1:r_list[num_m], , num_m] = matrix(chsi[, 1:r_list[num_m], 1:r_list[num_m], num_m], ncol = n) + roy[1, 1:r_list[num_m], , num_m] ^
            2
          if (change) {
            sigma.tot[1, 1:p[num_m], 1:p[num_m], num_m] = var(X[, 1:p[num_m], num_m])
          }
        } else{
          for (i in 1:k) {
            chsi[i, 1:r_list[num_m], 1:r_list[num_m], num_m] <-
              ginv(t(H[1:p[num_m], 1:r_list[num_m], num_m]) %*% ginv(psi[1:p[num_m], 1:p[num_m], num_m]) %*%
                     H[1:p[num_m], 1:r_list[num_m], num_m] + ginv(sigma[i, 1:r_list[num_m], 1:r_list[num_m], num_m]))
            if (!is.null(Z)) {
              roy[i, 1:r_list[num_m], , num_m] <-
                chsi[i, 1:r_list[num_m], 1:r_list[num_m], num_m] %*% (t(H[1:p[num_m], 1:r_list[num_m], num_m]) %*%
                                                                        ginv(psi[1:p[num_m], 1:p[num_m], num_m]) %*% t(X[, 1:p[num_m], num_m] - Z %*% t(B[1:p[num_m], , num_m])) +
                                                                        ginv(sigma[i, 1:r_list[num_m], 1:r_list[num_m], num_m]) %*% beta[i, 1:r_list[num_m], num_m] %*%
                                                                        t(x.z))
            } else{
              roy[i, 1:r_list[num_m], , num_m] <-
                chsi[i, 1:r_list[num_m], 1:r_list[num_m], num_m] %*% (t(H[1:p[num_m], 1:r_list[num_m], num_m]) %*%
                                                                        ginv(psi[1:p[num_m], 1:p[num_m], num_m]) %*% t(X[, 1:p[num_m], num_m]) +
                                                                        ginv(sigma[i, 1:r_list[num_m], 1:r_list[num_m], num_m]) %*% beta[i, 1:r_list[num_m], num_m] %*%
                                                                        t(x.z))
            }
            if (r_list[num_m] > 1) {
              temp <-
                (t(roy[i, 1:r_list[num_m], , num_m])) %o% (roy[i, 1:r_list[num_m], , num_m])
              temp <- apply(temp, c(2, 3), diag)#####
              temp <- aperm(temp, c(2, 3, 1))##########
              temp2 <-
                array(chsi[i, 1:r_list[num_m], 1:r_list[num_m], num_m], c(r_list[num_m], r_list[num_m], n))###########
              Ezz.hy[i, 1:r_list[num_m], 1:r_list[num_m], , num_m] <-
                temp + temp2
            } else{
              Ezz.hy[i, r_list[num_m], r_list[num_m], , num_m] <-
                roy[i, 1:r_list[num_m], , num_m] ^ 2 + rep(chsi[i, 1:r_list[num_m], 1:r_list[num_m], num_m], n)
            }
            if (change[i]) {
              sigma.tot[i, 1:p[num_m], 1:p[num_m], num_m] <-
                var(X[, 1:p[num_m], num_m])
            }#########OIS
      ...
      for (num_n in 1:n) {
        for (i in 1:k) {
          ...
          for (num_m in 1:m) {
            #cat(i, num_m, "\n")
            resid_m = X[num_n, 1:p[num_m], num_m] - t(as.matrix(H[1:p[num_m], 1:r_list[num_m], num_m]) %*%
                                                        beta[i, 1:r_list[num_m], num_m]) - Z[num_n, ] %*% t(B[1:p[num_m], , num_m])
            sigma_m = sigma.tot[i, 1:p[num_m], 1:p[num_m], num_m]
            #dim(resid_m)
            #dim(solve(sigma_m))
            #(2*pi)^(-p/2) * det(sigma_m)^(-1/2)*exp(-1/2 * resid_m %*% as.matrix(solve(sigma_m)) %*% t(resid_m))
            temp_p_2 = dmvnorm(resid_m,
                               mean = rep(0, p[num_m]),
                               sigma = as.matrix(sigma_m))
            #print(resid_m)
            #print(sigma_m)
            #cat(temp_p_2, "\n")
    ...
    if (!is.null(Xc)) {
      ...
      for (i in 1:mc) {
        cat(i, '\n')
        e_ordinal = list.append(e_ordinal,
                                ordinal_estep(x = Xc[[i]], init_ordinal = init_ordinal[[i]]))
      }
    }
    
    #**************************************************************************************************************************
    ##############for mixed modalities#########
    py.h_mixed = py.s_mixed = py.s_mixed2 = array(1, c(k, n))
    py.h_total_mixed = 0
    if (!is.null(X)) {
      py.h_mixed = py.h
    ...
    p.mix.y = modality_mixed(init_ordinal,
                             py.h_mixed,
                             py.h_total_mixed,
                             py.s_mixed,
                             k,
    ...
    if (!is.null(Xc)) {
      for (i in 1:mc) {
        #e_ordinal[[i]]=ordinal_estep2(init_ordinal=init_ordinal[[i]], e_ordinal[[i]], p.mix.y)
        e_ordinal[[i]] = Ez.y_mixed(e_ordinal[[i]],
    ...
    W = W_mixed(p.mix.y = p.mix.y$p.mix.y, n, k)
    for (i in 1:mc) {
    ...
    if (!is.null(X)) {
      ...
      for (num_m in 1:m) {
        for (i in 1:k) {
          Ez.y[1:r_list[num_m], , num_m] <-
            Ez.y[1:r_list[num_m], , num_m] + t(matrix(ph.y[i,], n, r_list[num_m])) *
            Ez.hy[i, 1:r_list[num_m], , num_m]
          Ezz.y[1:r_list[num_m], 1:r_list[num_m], , num_m] <-
            Ezz.y[1:r_list[num_m], 1:r_list[num_m], , num_m] + aperm(array(ph.y[i,], c(n, r_list[num_m], r_list[num_m])), c(2, 3, 1)) *
            Ezz.hy[i, 1:r_list[num_m], 1:r_list[num_m], , num_m]
        }
        # 2/11/2017 11:16PM
        ...
        if (r_list[num_m] != 1) {
          Ez.y[1:r_list[num_m], , num_m] <-
            ifelse(is.na(Ez.y[1:r_list[num_m], , num_m]), rowMeans(Ez.y[1:r_list[num_m], , num_m], na.rm =
                                                                     TRUE), Ez.y[1:r_list[num_m], , num_m])
          Ezz.y[1:r_list[num_m], 1:r_list[num_m], , num_m] <-
            ifelse(is.na(Ezz.y[1:r_list[num_m], 1:r_list[num_m], , num_m]), rowMeans(Ezz.y[1:r_list[num_m], 1:r_list[num_m], , num_m], na.rm =
                                                                                       TRUE), Ezz.y[1:r_list[num_m], 1:r_list[num_m], , num_m])
        }
      }
      ...
      for (num_m in 1:m) {
        ...
        B = as.vector(t(as.matrix(cbind(
          t(X[, 1:p[num_m], num_m]) %*% t(array(Ez.y[1:r_list[num_m], , num_m], c(r_list[num_m], n))), t(X[, 1:p[num_m], num_m]) %*% t(array(t(Z), c(q, n)))
        ))))
        A = as.matrix(cbind(rbind(n * EEzz.y, t(Z) %*% t(
          array(Ez.y[1:r_list[num_m], , num_m], c(r_list[num_m], n))
        )), rbind(t(
          t(Z) %*% t(array(Ez.y[1:r_list[num_m], , num_m], c(r_list[num_m], n)))
        ), t(Z) %*% Z)))
        D_temp = tryCatch({
          (chol(A))
        },
        error = function(cond) {
          message("A is not positive define")
          return(A)
        })
      ...
      fit_H = gglasso(
        ...
        group = group_gamma,
        loss = "ls",
        lambda = lambda_gamma / nrow(X_gamma),
        pf = pf_gamma,
        intercept = FALSE
      )$beta
      ...
      for (num_m in 1:m) {
        ...
        psi[1:p[num_m], 1:p[num_m], num_m] = (t(X[, 1:p[num_m], num_m] - Z %*% t(B[1:p[num_m], , num_m])) %*% (X[, 1:p[num_m], num_m] - Z %*% t(B[1:p[num_m], , num_m])) - t(X[, 1:p[num_m], num_m] - Z %*% t(B[1:p[num_m], , num_m])) %*% t(array(Ez.y[1:r_list[num_m], , num_m], c(r_list[num_m], n))) %*% t(H[1:p[num_m], 1:r_list[num_m], num_m])) /
          n
      ...
      # }
      # for(i in 1:dim(py.h)[1]){
      #   for(j in 1:dim(py.h)[2]){
      #     py.h[i, j] = ifelse(py.h[i, j] == 0, 1e-10, py.h[i, j])
      #     py.h[i, j] = ifelse(is.na(py.h[i, j]), 1e-10, py.h[i, j])
      #     py.h[i, j] = ifelse(is.infinite(py.h[i, j]), 1e+100, py.h[i, j])
      #   }
      # }
      
      
      temp_numeric = sum(log(ifelse(
        colSums(W * py.h * 10 ^ py.h_total) == 0,
        1e-300,
        colSums(W * py.h * 10 ^ py.h_total)
      ))) + sum(py.h_pro[k, ]) * log(10)
    ...
    if (!is.null(Xc)) {
      ...
      for (i in 1:mc) {
        print(i)
        m_ordinal = list.append(
          m_ordinal,
          ordinal_mstep(
            p.mix.y = p.mix.y$p.mix.y ,
            e_ordinal = e_ordinal[[i]],
            init_ordinal = init_ordinal[[i]],
            lambda_cnlm
          )
        )
    ...
    if (!is.null(Xc) & !is.null(X)) {
      res = modality_selection(
        p.mix.y_input = p.mix.y,
        init_ordinal,
        e_ordinal,
        m_ordinal,
        mc,
    ...
    if (!is.null(Xc) & is.null(X)) {
      res = modality_selection2(p.mix.y = p.mix.y,
                                init_ordinal,
                                e_ordinal,
                                m_ordinal,
                                mc,
    ...
    ratio = ifelse(is.na(abs((temp_total - lik) / lik)), 0, abs((temp_total - lik) /
                                                                  lik))
    ...
    for (i in 1:mc) {
      ...
      muc_selected = cbind(muc_selected, muc)
  ...
  # for (i in 1:mc) {
  #   alpha = ifelse(abs(m_ordinal[[i]]$init_ordinal$alpha) < 1e-3,
  #                  0,
  #                  m_ordinal[[i]]$init_ordinal$alpha)
  #   
  #   fit$alpha = cbind(fit$alpha, alpha)
  # 
  #   fit$alpha2 = cbind(fit$alpha2, m_ordinal[[i]]$init_ordinal$alpha2)
  # 
  #   fit$Sensitivity[i] = ifelse(sum(m_ordinal[[i]]$init_ordinal$muc  != 0) !=
  #                                 0,
  #                               sum(alpha[1:(init_ordinal[[i]]$pc / 4), ] != 0) / (rc * (init_ordinal[[i]]$pc / 4) - 1),
  #                               NA)
  #   fit$Specificity[i] = ifelse(sum(m_ordinal[[i]]$init_ordinal$muc  != 0) !=
  #                                 0,
  #                               sum(alpha[(init_ordinal[[i]]$pc / 4 + 1):init_ordinal[[i]]$pc,] == 0) / (rc * (init_ordinal[[i]]$pc - init_ordinal[[i]]$pc / 4)),
  #                               NA)
  # }
  # 
  # for (num_m in (mc + 1):(mc + m)) {
  #   fit$Sensitivity[num_m] = sum(H[1:(p_list[num_m - mc] / 4), 1:r_list[num_m - mc], num_m - mc] != 0) / (rc * (p_list[num_m - mc] / 4))
  #   fit$Specificity[num_m] = sum(H[(p_list[num_m - mc] / 4 + 1):p_list[num_m - mc], 1:r_list[num_m - mc], num_m - mc] == 0) / (rc * (p_list[num_m - mc] - (p_list[num_m - mc] / 4)))
  # }
  ...
  if (!is.null(Xc)) {
    # h = h + sum(fit$sigmac != 0) + sum(fit$muc != 0) - (rc * (rc + 1) / 2 + rc) *
    #   mc + sum(fit$alpha != 0) + sum((j_list - 1)[1:init_ordinal[[1]]$pc]) * mc + sum((j_list - 1)[1:init_ordinal[[1]]$pc]) *
    #   mc * init_ordinal[[1]]$pc
